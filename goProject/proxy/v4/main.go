...
func connect(reader *bufio.Reader, conn net.Conn) (err error) {
	...
	//需要真正的和IP端口建立TCP连接，双向转换数据
	//使用net.Dial函数与指定IP和端口进行TCP连接
	...
	//标准库的context机制，使用context.WithCancel创建一个context
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	//io.Copy(dst Writer, src Reader)死循环从src里面尝试读取数据向dst内写入数据
	//由于我们是双向通信，所以需要两个go routinue,
	//一个从用户浏览器拷贝数据到底层服务器，一个从底层服务器拷贝数据到用户浏览器。
	go func() {
	...
	//等待context执行完成，执行完成的时机也是cancel的时机
	//这样就实现了任何一个方向上的函数执行失败（断开连接）就退出函数并关闭连接
	<-ctx.Done()
	return nil
}